const data = [
    {
      name: "BC Cancer Foundation",
      title: "Wayfinding Assistance",
      description: "Help families and patients find their way to appointments and meetings.",
      daysPerWeek: "5",
      hoursPerShift: "3",
      termLength: "6 months",
      longDescription: "BC Cancer Foundation funds life-saving cancer research and patient care in British Columbia, supporting prevention, treatment, and innovation to improve outcomes for those affected by cancer.",
      category: "Medical assistance",
      donatelink: "https://bccancerfoundation.com/donations/donations-changes-everything/?s_src=googleEvergreenH&utm_medium=cpc&utm_source=googleEvergreenH&utm_campaign=EOY25&gad_source=1&gad_campaignid=23273507695&gbraid=0AAAAADwT5GGWwi7kvCH7utaYHz2gqryuv&gclid=Cj0KCQiAiebIBhDmARIsAE8PGNLSeAryD1_dh_puvGg_4PS8MmKdnrthupi_9Hj20CSS_eBNmLgWT5caAncjEALw_wcB",
      applylink: "https://www.bccancer.bc.ca/our-services/centres-clinics/bc-cancer-vancouver/volunteer",
      lat: 49.263279,
      lng: -123.120310,
      img: "data:image/png;base64,",
      categories: [
        "Hospital",
        "Medical assistance",
      ],
      needs: [
        "Positive attitude",
        "18+ age",
        "Teamwork",
      ]
    },
    {
      name: "Vancouver Food Bank",
      title: "Food Delivery Driver",
      description: "Deliver meals to those who need support.",
      daysPerWeek: "2",
      hoursPerShift: "4",
      termLength: "2 months",
      longDescription: "Vancouver Food Bank provides essential food and support to individuals and families in need, working to reduce hunger and strengthen community well-being across the city.",
      donatelink: "https://foodbank.bc.ca",
      applylink: "https://foodbank.bc.ca/volunteer/individual-volunteers/",
      category: "Food security",
      lat: 49.282630,
      lng: -123.134956,
      img: "data:image/png;base64,",
      categories: [
        "Food Bank",
        "Delivery Driver"
      ],
      needs: [
        "Ability to drive",
        "Can-do Attitude",
        "Adapt on the go"
      ]
    },
    {
        name: "Parkinson's Canada",
        title: "Support Group Facilitator",
        description: "Attend support groups and help attendees.",
        daysPerWeek: "3",
        hoursPerShift: "2",
        termLength: "5 months",
        longDescription: "Parkinson Canada’s mission is to improve the quality of life for people living with Parkinson’s through support, education, research, and advocacy for better care and treatments.",
        donatelink: "https://donate.parkinson.ca",
        applylink: "https://www.parkinson.ca/get-involved/volunteer/",
        category: "Medical assistance",
        lat: 49.287323,
        lng: -123.117493,
        img: "data:image/png;base64,",
        categories: [
            "Social support",
            "Medical support",
        ],
        needs: [
            "Leadership qualities",
            "Ability to have hard conversations"
        ]
    },
    {
        name: "RainCity Housing",
        title: "Community Outreach Volunteer",
        description: "Support individuals experiencing homelessness by distributing resources and providing guidance.",
        daysPerWeek: "3",
        hoursPerShift: "4",
        termLength: "4 months",
        longDescription: "RainCity Housing provides housing and support services to vulnerable populations, aiming to end homelessness and improve the well-being of those they serve.",
        category: "Social services",
        donatelink: "https://raincityhousing.org/donate",
        applylink: "https://raincityhousing.org/volunteer",
        lat: 49.2819,
        lng: -123.1207,
        img: "https://www.raincityhousing.org/wp-content/uploads/2017/09/raincity-id.jpg",
        categories: [
            "Helping homeless",
            "Personal service"
        ],
        needs: [
            "Supportive and approachable",
            "Adapt on the go"
        ]
    },
    {
        name: "Vancouver Humane Society",
        title: "Animal Care Volunteer",
        description: "Assist with feeding, cleaning, and socializing animals at the shelter.",
        daysPerWeek: "2",
        hoursPerShift: "3",
        termLength: "6 months",
        longDescription: "The Vancouver Humane Society works to protect animals through rescue, rehabilitation, and adoption programs, while promoting responsible pet ownership.",
        category: "Animal welfare",
        donatelink: "https://vancouverhumanesociety.bc.ca/donate",
        applylink: "https://vancouverhumanesociety.bc.ca/volunteer",
        lat: 49.2827,
        lng: -123.1207,
        img: "https://media.licdn.com/dms/image/v2/C560BAQHv8s7zzMvNAQ/company-logo_200_200/company-logo_200_200/0/1630640468975/vancouver_humane_society_logo?e=2147483647&v=beta&t=i3xCfkJ5xfxIFYE_m89cj6YVL45VkvepRWxRHqpiDJ8",
        categories: [
            "Animal care",
            "Animal shelter"
        ],
        needs: [
            "Caring and personable",
            "Animal lover",
            "No allergies"
        ]
    },
    {
        name: "Food Runners Vancouver",
        title: "Food Delivery Volunteer",
        description: "Pick up surplus food from restaurants and deliver it to local shelters and food banks.",
        daysPerWeek: "1-2",
        hoursPerShift: "2-3",
        termLength: "Ongoing",
        longDescription: "Food Runners Vancouver reduces food waste and addresses hunger by redistributing surplus food from local businesses to community organizations in need.",
        category: "Food security",
        donatelink: "https://foodrunners.ca/donate",
        applylink: "https://foodrunners.ca/volunteer",
        lat: 49.2636,
        lng: -123.1386,
        img: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR_RhMmwUTLQVc6a3pz2_FQ44UiGpr_J1Y-ag&s",
        categories: [
            "Food Bank",
            "Delivery Driver"
        ],
        needs: [
            "Personable",
            "Can-do Attitude",
            "Adapt on the go"
        ]
    }
  ]

  export default data